...
export class CellRenderer extends LabelButton implements ICellRenderer
  ...
  [key: string]: any;
      ...
      {
         return CellRenderer.defaultStyles;
      }
      ...
      protected drawLayout(): void
      {
         ...
         if(icon != null)
         {
            (icon as any).x = _loc1_;
            (icon as any).y = Math.round(height - icon.height >> 1);
            var _loc2_: number = icon.width + _loc1_;
         }
         if(label.length > 0)
         {
            (textField as any).visible = true;
            _loc3_ = Math.max(0,width - 0 - _loc1_ * 2);
            (textField as any).width = _loc3_;
            (textField as any).height = textField.textHeight + 4;
            (textField as any).x = 0 + _loc1_;
            (textField as any).y = Math.round(height - textField.height >> 1);
         }
         else
         {
            (textField as any).visible = false;
         }
