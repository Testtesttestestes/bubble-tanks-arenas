...
export class class_119 extends class_68{
  [key: string]: any;
      ...
      public TestForCollision(): boolean
      {
         ...
         {
            if(objBFRef.arrBullets[_loc4_ as any].blnEnemy)
            {
               var _loc3_: Point = new (Point as any)(objBFRef.arrBullets[_loc4_ as any].x,objBFRef.arrBullets[_loc4_ as any].y);
               _loc3_ = objBFRef.arrBullets[_loc4_ as any].parent.localToGlobal(null);
               (_loc2_ as any).x = (null as any).x - _loc1_.x;
               (_loc2_ as any).y = (null as any).y - _loc1_.y;
               if(_loc2_.method_41() < 11[5] + objBFRef.arrBullets[_loc4_ as any].width / 2)
               {
                  objBFRef.arrBullets[_loc4_ as any].intState = 2;
                  objBFRef.arrBullets[_loc4_ as any].moveX = moveX;
                  objBFRef.arrBullets[_loc4_ as any].moveY = moveY;
                  return true;
               }
            }
            _loc4_ += 1;
