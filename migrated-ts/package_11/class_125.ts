...
import { seeker_explosion } from '../seeker_explosion';
...
export class class_125 extends class_68{
  [key: string]: any;
      ...
      public method_623(): boolean
      {
         ...
         {
            _loc4_ = new (Point as any)(this.x,this.y);
            _loc4_ = this.parent.localToGlobal(_loc4_);
            _loc5_ = new (Point as any)(objBFRef.arrEnemies.x,objBFRef.arrEnemies.y);
            _loc5_ = this.parent.localToGlobal(_loc5_);
            _loc4_ = new (Point as any)(_loc4_.x - _loc5_.x,_loc4_.y - _loc5_.y);
         ...
         var_34 = objBFRef.arrEnemies[_loc2_ as any];
         return true;
      }
      
      public RotateTowardTarget(): number
      {
         var var_34: any, var_31: any;

         if(var_34 == null || !objBFRef.contains(var_34))
         {
            if(!this.method_623())
            {
               objBFRef.arrBulletRemoveQueue.push(this as any);
               var_31 = false;
         ...
         var _loc2_: Point = new (Point as any)(var_34.GetHitLocation().x,var_34.GetHitLocation().y);
      ...
      {
         objBFRef.AddEffectToQueue(new (seeker_explosion as any)(),this.x,this.y,"TopEffectsLayer",true);
         if(objWorld.intHitTimer == -1)
         {
            objWorld.mRef.SoundCall(new (class_49 as any)());
