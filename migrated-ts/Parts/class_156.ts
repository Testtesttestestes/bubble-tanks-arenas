...
import { class_83 } from '../class_83';
import { class_122 } from '../package_11/class_122';
...
// @ts-ignore
export class class_156 extends Weapon{
  [key: string]: any;
      ...
      public PartCallTrigger(): void
      {
         ...
         arrMineRefs.push({
            "mc":var_3.objData.objWorld.currentBFTarget.SpawnNewBullet(new (class_122 as any)(),_loc1_.x,_loc1_.y,1,this.rotation + var_3.rotation,10,10,true),
            "x":_loc1_.x,
            "y":_loc1_.y
         } as any);
         if(arrMineRefs.length == 8)
         {
            var_3.objData.objWorld.currentBFTarget.AddEffectToQueue(new (class_83 as any)(),arrMineRefs[0].x,arrMineRefs[0].y,"BottomEffectsLayer",false);
            _loc2_ = 0;
            while(_loc2_ < var_3.objData.objWorld.currentBFTarget.arrBulletRemoveQueue.length)
            {
               if(var_3.objData.objWorld.currentBFTarget.arrBulletRemoveQueue[_loc2_ as any] == arrMineRefs[0].mc)
               {
