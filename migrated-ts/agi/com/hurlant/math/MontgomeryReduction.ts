...
// @ts-ignore
  ...
  [key: string]: any;
      ...
      public reduce(x: BigInteger): void
      {
         ...
         {
            j = x.a[i as any] & 0x7FFF;
            u0 = j * this.mpl + ((j * this.mph + (x.a[i as any] >> 15) * mpl & um) << 15) & BigInteger.DM;
            j = i + this.m.t;
            x.a[j as any] += this.m.am(0,u0,x,i,0,this.m.t);
            while(x.a[j as any] >= BigInteger.DV)
            {
               x.a[j as any] -= BigInteger.DV;
               ++x.a[++j as any];
            }
         }
         x.clamp();
