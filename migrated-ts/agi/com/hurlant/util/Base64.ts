...
// @ts-ignore
  ...
  [key: string]: any;
      ...
      public static encodeByteArray(data: ByteArray): string
      {
         ...
         var k: number = 0;
         output = "";
         outputBuffer = new (Array as any)(4);
         data.position = 0;
         while(data.bytesAvailable > 0)
         {
            dataBuffer = new (Array as any)();
            i = 0;
            while(i < 3 && data.bytesAvailable > 0)
            {
               dataBuffer[i as any] = data.readUnsignedByte();
               i++;
            }
            ...
            {
               outputBuffer[j + 1 as any] = 64;
            }
            for(k = 0; k < outputBuffer.length; k++)
            {
               output += Base64.BASE64_CHARS.charAt(outputBuffer[k as any]);
            }
         }
         return output;
      }
      ...
      public static decodeToByteArray(data: string): ByteArray
      {
         ...
         var j: number = 0;
         var k: number = 0;
         output = new (ByteArray as any)();
         dataBuffer = new (Array as any)(4);
         outputBuffer = new (Array as any)(3);
         for(i = 0; i < data.length; i += 4)
         {
            j = 0;
            while(j < 4 && i + j < data.length)
            {
               dataBuffer[j as any] = Base64.BASE64_CHARS.indexOf(data.charAt(i + j));
               j++;
            }
            ...
            for(k = 0; k < outputBuffer.length; k++)
            {
               if(dataBuffer[k + 1 as any] == 64)
               {
                  break;
               }
               output.writeByte(outputBuffer[k as any]);
            }
         }
         output.position = 0;
         return output;
