...
// @ts-ignore
  ...
  [key: string]: any;
      ...
      public dispose(): void
      {
         ...
         {
            this.k[i as any] = r.nextByte();
            delete this.k[i as any];
         }
         this.k = null as any;
         Memory.gc();
      }
      ...
      public encrypt(block: ByteArray, index: number = 0): void
      {
         ...
         {
            v0 += (v1 << 4 ^ v1 >> 5) + v1 ^ sum + this.k[sum & 3 as any];
            sum += delta;
            v1 += (v0 << 4 ^ v0 >> 5) + v0 ^ sum + this.k[sum >> 11 & 3 as any];
         }
         block.position -= 8;
