...
export class DESKey implements ISymmetricKey
   {
      ...
      protected generateWorkingKey(encrypting: boolean, key: ByteArray, off: number): any[]
      {
         ...
         for(i = 0; i != 32; i += 2)
         {
            i1 = Math.floor(newKey[i]);
            i2 = Math.floor(newKey[i + 1]);
      ...
      protected desFunc(wKey: any[], inp: ByteArray, inOff: number, out: ByteArray, outOff: number): void
      {
         ...
         right = Math.floor((inp[inOff + 4] & 0xFF) << 24);
         right |= (inp[inOff + 5] & 0xFF) << 16;
         right |= (inp[inOff + 6] & 0xFF) << 8;
         right |= inp[inOff + 7] & 0xFF;
         work = Math.floor((left >>> 4 ^ right) & 0x0F0F0F0F);
         right ^= work;
         left ^= work << 4;
         work = Math.floor((left >>> 16 ^ right) & 0xFFFF);
         right ^= work;
         left ^= work << 16;
         work = Math.floor((right >>> 2 ^ left) & 0x33333333);
         left ^= work;
         right ^= work << 2;
         work = Math.floor((right >>> 8 ^ left) & 0xFF00FF);
         left ^= work;
         right ^= work << 8;
         right = Math.floor((right << 1 | right >>> 31 & 1) & 0xFFFFFFFF);
         work = Math.floor((left ^ right) & 0xAAAAAAAA);
         left ^= work;
         right ^= work;
         left = Math.floor((left << 1 | left >>> 31 & 1) & 0xFFFFFFFF);
         ...
         left ^= work;
         right ^= work;
         left = Math.floor(left << 31 | left >>> 1);
         work = Math.floor((left >>> 8 ^ right) & 0xFF00FF);
         right ^= work;
         left ^= work << 8;
         work = Math.floor((left >>> 2 ^ right) & 0x33333333);
         right ^= work;
         left ^= work << 2;
         work = Math.floor((right >>> 16 ^ left) & 0xFFFF);
         left ^= work;
         right ^= work << 16;
         work = Math.floor((right >>> 4 ^ left) & 0x0F0F0F0F);
         left ^= work;
         right ^= work << 4;
