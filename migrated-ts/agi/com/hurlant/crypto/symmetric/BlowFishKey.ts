...
// @ts-ignore
  ...
  [key: string]: any;
      ...
      private static readonly KP: any = [608135816,2242054355,320440878,57701188,2752067618,698298832,137296536,3964562569,1160258022,953160567,3193202383,887688300,3232508343,3380367581,1065670069,3041331479,2450970073,2306472731];
      ...
      private static readonly P_SZ: number = BlowFishKey.ROUNDS + 2;
      private S3!: any;
      private S2!: any;
      private P!: any;
      ...
      {
         return(this.S0[x >>> 24] + this.S1[x >>> 16 & 0xFF] ^ this.S2[x >>> 8 & 0xFF]) + this.S3[x & 0xFF as any];
      }
      
      private BytesTo32bits(b: ByteArray, i: number): number
      {
         return(b[i] & 0xFF) << 24 | (b[i + 1 as any] & 0xFF) << 16 | (b[i + 2 as any] & 0xFF) << 8 | b[i + 3 as any] & 0xFF;
      }
      ...
      private decryptBlock(src: ByteArray, srcIndex: number, dst: ByteArray, dstIndex: number): void
      {
         ...
         var xr: number = 0;
         var i: number = 0;
         xl = this.BytesTo32bits(src,srcIndex);
         xr = this.BytesTo32bits(src,srcIndex + 4);
         xl ^= this.P[BlowFishKey.ROUNDS + 1 as any];
         for(i = BlowFishKey.ROUNDS; i > 0; i -= 2)
         {
            xr ^= this.F(xl) ^ this.P[i as any];
            xl ^= this.F(xr) ^ this.P[i - 1 as any];
         }
         xr ^= this.P[0];
      ...
      private setKey(key: ByteArray): void
      {
         ...
         keyIndex = 0;
         for(i = 0; i < BlowFishKey.P_SZ; i++)
         {
            data = 0;
            for(j = 0; j < 4; j++)
            {
               data = uint(data << 8 | key[keyIndex++ as any] & 0xFF);
               if(keyIndex >= keyLength)
               {
                  keyIndex = 0;
               }
            }
            this.P[i as any] ^= data;
         }
      ...
      private encryptBlock(src: ByteArray, srcIndex: number, dst: ByteArray, dstIndex: number): void
      {
         ...
         xr = this.BytesTo32bits(src,srcIndex + 4);
         xl ^= this.P[0];
         for(i = 1; i < BlowFishKey.ROUNDS; i += 2)
         {
            xr ^= this.F(xl) ^ this.P[i as any];
            xl ^= this.F(xr) ^ this.P[i + 1 as any];
         }
         xr ^= this.P[BlowFishKey.ROUNDS + 1 as any];
         this.Bits32ToBytes(xr,dst,dstIndex);
         this.Bits32ToBytes(xl,dst,dstIndex + 4);
      }
      ...
      private Bits32ToBytes(i: number, b: ByteArray, offset: number): void
      {
         b[offset + 3 as any] = i;
         b[offset + 2 as any] = i >> 8;
         b[offset + 1 as any] = i >> 16;
         b[offset as any] = i >> 24;
      }
