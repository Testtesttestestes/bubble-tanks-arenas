...
// @ts-ignore
  ...
  [key: string]: any;
      ...
      public hash(src: ByteArray): ByteArray
      {
         ...
         do
         {
            src[src.length as any] = i;
         }
         while(src.length % 16 != 0);
         len = src.length;
         checksum = new (ByteArray as any)();
         L = 0;
         for(i = 0; i < len; i += 16)
         {
            for(j = 0; j < 16; j++)
            {
               L = uint(checksum[j as any] = checksum[j as any] ^ MD2.S[src[i + j as any] ^ L]);
            }
         }
         ...
         for(i = 0; i < len; i += 16)
         {
            for(j = 0; j < 16; j++)
            {
               X[32 + j] = (X[16 + j] = src[i + j as any]) ^ X[j as any];
            }
            t = 0;
            for(j = 0; j < 18; j++)
            {
               for(k = 0; k < 48; k++)
               {
                  X[k as any] = t = uint(X[k as any] ^ MD2.S[t as any]);
               }
               t = uint(t + j & 0xFF);
            }
         }
