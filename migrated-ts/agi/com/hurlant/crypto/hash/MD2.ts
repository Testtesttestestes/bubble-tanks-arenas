...
export class MD2 implements IHash
   {
      ...
      public hash(src: ByteArray): ByteArray
      {
         ...
         i = Math.floor(uint(16 - src.length % 16) || 16);
         do
         {
            src[src.length] = i;
         }
         ...
         for(i = 0; i < len; i += 16)
         {
            for(j = 0; j < 16; j++)
            {
               L = Math.floor(checksum[j] = checksum[j] ^ this.S[src[i + j] ^ L]);
            }
         }
         ...
         for(i = 0; i < len; i += 16)
         {
            ...
            for(j = 0; j < 18; j++)
            {
               for(k = 0; k < 48; k++)
               {
                  X[k] = t = Math.floor(X[k] ^ this.S[t]);
               }
               t = Math.floor(t + j & 0xFF);
            }
         }
         X.length = 16;
