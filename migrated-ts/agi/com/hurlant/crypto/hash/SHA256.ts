...
// @ts-ignore
  ...
  [key: string]: any;
      ...
      protected core(x: any, len: number): any
      {
         ...
         var s0: number = 0;
         var s1: number = 0;
         x[len >> 5 as any] |= 128 << 24 - len % 32;
         x[(len + 64 >> 9 << 4) + 15 as any] = len;
         w = [];
         a = uint(this.h[0]);
         b = uint(this.h[1]);
         c = uint(this.h[2]);
         ...
         for(i = 0; i < x.length; i += 16)
         {
            ...
            for(j = 0; j < 64; j++)
            {
               if(j < 16)
               {
                  w[j as any] = x[i + j as any] || 0;
               }
               else
               {
                  s0 = uint(this.rrol(w[j - 15 as any],7) ^ this.rrol(w[j - 15 as any],18) ^ w[j - 15 as any] >>> 3);
                  s1 = uint(this.rrol(w[j - 2 as any],17) ^ this.rrol(w[j - 2 as any],19) ^ w[j - 2 as any] >>> 10);
                  w[j as any] = w[j - 16 as any] + s0 + w[j - 7 as any] + s1;
               }
               t2 = (this.rrol(a,2) ^ this.rrol(a,13) ^ this.rrol(a,22)) + (a & b ^ a & c ^ b & c);
               t1 = h + (this.rrol(e,6) ^ this.rrol(e,11) ^ this.rrol(e,25)) + (e & f ^ g & ~e) + SHA256.k[j as any] + w[j as any];
               h = g;
               g = f;
               f = e;
               e = d + t1;
