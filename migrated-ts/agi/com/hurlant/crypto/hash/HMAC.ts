...
// @ts-ignore
  ...
  [key: string]: any;
      ...
      public compute(key: ByteArray, data: ByteArray): ByteArray
      {
         ...
         {
            hashKey = new (ByteArray as any)();
            hashKey.writeBytes(key);
         }
         while(hashKey.length < this.hash.getInputSize())
         {
            hashKey[hashKey.length as any] = 0;
         }
         innerKey = new (ByteArray as any)();
         outerKey = new (ByteArray as any)();
         for(i = 0; i < hashKey.length; i++)
         {
            innerKey[i as any] = hashKey[i as any] ^ 0x36;
            outerKey[i as any] = hashKey[i as any] ^ 0x5C;
         }
