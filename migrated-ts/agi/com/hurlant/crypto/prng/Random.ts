...
// @ts-ignore
  ...
  [key: string]: any;
      ...
      constructor(prng: any = null){
         ...
         this.state = new (prng as any)()  as unknown as IPRNG;
         this.psize = this.state.getPoolSize();
         this.pool = new (ByteArray as any)();
         this.pptr = 0;
         while(pptr < this.psize)
         {
            t = 65536 * Math.random();
            this.pool[pptr++ as any] = t >>> 8;
            this.pool[pptr++ as any] = t & 0xFF;
         }
         this.pptr = 0;
         this.seed();
      }
      ...
      public seed(x: number = 0): void
      {
         if(x == 0)
         {
            x = new (Date as any)().getTime();
         }
         var _temp_3: any = this.pool;
         var _loc2_: number = this.pptr++;
         _temp_3[_loc2_ as any] ^= x & 0xFF;
         var _temp_6: any = this.pool;
         var _loc3_: number = this.pptr++;
         _temp_6[_loc3_ as any] ^= x >> 8 & 0xFF;
         var _temp_9: any = this.pool;
         var _loc4_: number = this.pptr++;
         _temp_9[_loc4_ as any] ^= x >> 16 & 0xFF;
         var _temp_12: any = this.pool;
         var _loc5_: number = this.pptr++;
         _temp_12[_loc5_ as any] ^= x >> 24 & 0xFF;
      ...
      public dispose(): void
      {
         ...
         {
            this.pool[i as any] = Math.random() * 256;
         }
         this.pool.length = 0;
      ...
      public autoSeed(): void
      {
         ...
         b = new (ByteArray as any)();
         b.writeUnsignedInt(System.totalMemory);
         b.writeUTF(Capabilities.serverString);
         b.writeUnsignedInt(Date.now());
         b.writeUnsignedInt(new (Date as any)().getTime());
