  ...
  [key: string]: any;
      ...
      private arrayToString(param1: any): string
      {
         ...
         while(_loc3_ < param1.length)
         {
            ...
            _loc2_ += this.convertToString(param1[_loc3_ as any]);
            _loc3_++;
         }
         return "[" + _loc2_ + "]";
      }
      
      private objectToString(param1: any): string
      {
         var value: any = null as any;
         var key: string = null as any;
                  var o: any = param1;
         var s: string = "";
         for (let key in o) { value = o[key as any]; if (!(value instanceof Function)) { if (s.length > 0) { s += ","; } s += this.escapeString(key) + ":" + this.convertToString(value); } } return "{" + s + "}";
      }
   }
